...
# GIF-и
START_GIF_ID = "CgACAgIAAxkBAAIGaWklx4T4qipoaoAnQ-zZtONcI3PSAAKtggACY8QxSbsclWHIN-W8NgQ"  # START_ID

REMINDER_GIF_ID = "CgACAgIAAxkBAAIGZ2klx3L0zD481ukoCP5wcyZoU2RvAAK7ggACY8QxSVcrC1dM5txpNgQ"  # перший REMINDER_GIF
FEEDBACK_GIF_ID = "CgACAgIAAxkBAAIGb2klx8F-8H2XNpnZ_RkDngX68S2wAAKzggACY8QxSWy31mkhfyKuNgQ"   # другий REMINDER_GIF (для відгуків, якщо захочеш)

MENU_DONE_GIF_ID = "CgACAgIAAxkBAAIGa2klx6CGh8L5YmM8MHFLr7M1FxD2AAK4ggACY8QxSX7HU6jJHmdgNgQ"
ORG_CHAT_GIF_ID = "CgACAgIAAxkBAAIGbWklx7HmCZiIznwWnv9wXSUu6Tb4AAK2ggACY8QxSaGWcPcdf7p0NgQ"
SANTA_GIF_ID = "CgACAgIAAxkBAAIGcWklx9FKMEWywriPw7qL_GtwHaMYAAKxggACY8QxSThBFeYzRtypNgQ"
TASKS_GIF_ID = "CgACAgIAAxkBAAIGc2klx-Cwde-W_ZSRS5Af3GSxAR_NAAKrggACY8QxSeWebmRmuTecNgQ"
...
COLOR_TASKS: Dict[int, list[str]] = {
    1: [
        "• Похвали чийсь образ або деталь у дуже ніжному, м’якому стилі.",
        "• Зроби селфі так, щоб рожевий колір максимально домінував у кадрі.",
        "• Скажи речення тільки з іменників.",
        "• Почни хвалити чийсь образ, але скажи, що твій міг би бути кращим, якби не… (вигадати).",
        "• Хоч раз за вечір скажи: «Мені здається, тут не вистачає трохи ніжності».",
        *UNIVERSAL_TASKS,
    ],

    2: [
        "• Зобрази себе «Смурфика Святвечора» — смішна або мультяшна гримаса для фото.",
        "• Запропонуй мінімум двом людям вийти подихати або змінити локацію.",
        "• Зроби жартівливий прогноз погоди на вечір.",
        "• Знайди когось у теплому одязі й скажи, що він/вона як герой зимового мультика.",
        "• «Розморозь» когось сором’язливого, втягнувши у легку гру чи розмову.",
        *UNIVERSAL_TASKS,
    ],

    3: [
        "• Принеси щось мандаринове — аромат, жарт або асоціацію.",
        "• Запропонуй комусь тост «за аромат справжнього Різдва».",
        "• Розкажи коротку історію про свята та мандарини.",
        "• Пожартуй, що сьогодні пахнеш як… (вигадати).",
        "• Запитай про улюблений зимовий напій.",
        *UNIVERSAL_TASKS,
    ],

    4: [
        "• Зроби відео, як танцюєш біля ялинки.",
        "• Дозволь комусь «прикрасити» тебе, додавши іграшку.",
        "• Збери невелику компанію та зроби веселий кадр.",
        "• Тричі поспіль відповідай гостям однаковою фразою: «тиц пердиць».",
        "• Зроби момент «тихого лісу» — різко замовкни посеред історії.",
        *UNIVERSAL_TASKS,
    ],

    5: [
        "• Зроби вигляд, що образилась, і відповідай «+1 завдання» на питання що сталось.",
        "• Попроси когось погріти тобі руки.",
        "• Тричі поспіль відповідай одному гостю: «що це за завдання, я виграла!».",
        "• Зроби два фото, ніби дивитесь на зоряне небо.",
        "• Скажи хоча б двом людям, що вони виглядають краще, ніж нічні вогні.",
        *UNIVERSAL_TASKS,
    ],

    6: [
        "• Пожартуй, що ти офіційний постачальник гарного настрою.",
        "• Проведи «тест смаку» — попроси оцінити 5+ страв.",
        "• Поправ одяг хоча б трьом людям.",
        "• Попроси сфотографувати тебе біля ялинки й одразу скинути фото.",
        "• Зроби дивний комплімент одній з невісток (коли подякує — виконано).",
        *UNIVERSAL_TASKS,
    ],

    7: [
        "• Запропонуй уявну чашку гарячого шоколаду й опиши її смак.",
        "• Розкажи коротку історію, де какао — символ затишку.",
        "• Зроби фото або відео, де ви цокаєтесь.",
        "• Попроси десерт, а потім передумай і відмовся.",
        "• Скажи фразу: «Якщо стане холодно — у мене є какао», а потім додай: «а ні, я його забула».",
        *UNIVERSAL_TASKS,
    ],
}
...
async def postmenu_followups(bot: Bot, user_id: int):
    ...
    try:
        ...
        await send_gif(msg, REMINDER_GIF_ID)
    except Exception as e:
...
async def cb_party_confirm_rules(callback: CallbackQuery):
    ...
    if color:
        ...
        # 1. Образ
        first_text = (
            "Чудово! Тоді лови свій готовий образ 😊\n\n"
...
async def cb_fb_send(callback: CallbackQuery):
    user_id = callback.from_user.id
    user = get_user(user_id)
    await send_gif(callback.message, FEEDBACK_GIF_ID)
...
async def universal_handler(message: Message):
    ...
    if action == "set_dessert":
        ...
        await send_gif(message, MENU_DONE_GIF_ID)
        await asyncio.sleep(0.5)
        await message.answer(
